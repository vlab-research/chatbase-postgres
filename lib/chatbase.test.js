...
describe('Chatbase Postgres', () => {
  ...
  before(async () => {
    ...
    const messagesQuery = `create table if not exists messages(
			   userid VARCHAR NOT NULL,
			   timestamp TIMESTAMPTZ NOT NULL,
			   content VARCHAR NOT NULL,
                           hsh INT AS (fnv64a(content)) STORED NOT NULL,
			   PRIMARY KEY (hsh, userid));`
    await chatbase.pool.query(messagesQuery);
  ...
  describe('.get(key)', () => {
    ...
    it('does not throw on duplicate key error, but ignores it', async () => {
      await chatbase.put('foo', 'bar', new Date())
      await chatbase.put('foo', 'bar', new Date())
      const messages = await chatbase.get('foo')
      messages.length.should.equal(1)
      messages[0].should.equal('bar')
    })
